...
export default function StudentManagement() {
  ...
  // Filter students based on search query and program selection
  const filteredStudents = useMemo(() => {
    ...
    if (selectedProgramId) {
      filtered = filtered.filter(student => 
        student.programs.some(program => program.id === selectedProgramId)
      )
    }

    return filtered
  }, [students, debouncedSearchQuery, selectedProgramId])
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
                    ...
                    <td className="px-6 py-4">
                      {student.programs.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {student.programs.slice(0, 2).map((program) => (
                            <span
                              key={program.id}
                          ...
                          {student.programs.length > 2 && (
                            <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">
                              +{student.programs.length - 2} more
                            </span>
                          )}
                        </div>
                      ) : (
          ...
          <div className="bg-gray-50 px-6 py-3">
            <div className="text-sm text-gray-600">
              ...
              {selectedProgramId && ` enrolled in "${programs.find(p => p.id === selectedProgramId)?.name}"`}
            </div>
          </div>
        </div>
      )}
